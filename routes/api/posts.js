...
// @desc     Get post by ID
// @access   Private
router.get('/:id', [auth, check('id', 'ID is required').notEmpty()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.put('/like/:id', auth, async (req, res) => {
    ...
    try {
        ...
        /*
        post.likes：指的是当前帖子的“点赞”列表，这是一个数组。
        .unshift()是JavaScript数组的一个方法，用于在数组的开头添加一个或多个元素，并返回新的数组长度。
        {user: req.user.id}是要添加到数组开头的新元素，它是一个对象，其中包含一个user属性。这个user属性被设置为req.user.id的值，req.user.id很可能是当前发起请求的用户的ID。
        通过这种方式，新的点赞被添加到post.likes数组的最前面。这意味着如果你遍历post.likes数组，最新的点赞会是第一个出现的元素。
         */
